  ...
  module Alfalfa
    class Tagger
      ...
      def create_vav_reheat(air_loop, unit)
        vav_reheat = create_vav(air_loop, unit)
        # check reheat coil
        ...
        if rc.to_CoilHeatingWater.is_initialized
          ...
          if rc.plantLoop.is_initialized
            pl = rc.plantLoop.get
            vav_reheat[:hotWaterPlantRef] = create_ref(pl.handle)
          end
      ...
      def create_const_volume_reheat(air_loop, unit)
        const_vol_reheat = create_vav(air_loop, unit)
        # check reheat coil
        ...
        if rc.to_CoilHeatingWater.is_initialized
          ...
          if rc.plantLoop.is_initialized
            pl = rc.plantLoop.get
            const_vol_reheat[:hotWaterPlantRef] = create_ref(pl.handle)
          end
        elsif rc.to_CoilHeatingElectric.is_initialized
      ...
      def tag_thermal_zones
        @tz.each do |tz|
          if tz.name.is_initialized
            ...
            next if air_loops.empty?
            next if tz.isPlenum
            next if !tz.isConditioned
            thermal_zone_haystack = {}
      ...
      def tag_air_loops
        ...
        if !@ahu_components_tagged
          tag_air_loop_components
        end
        @air_loops.each do |air_loop|
          air_loop_typing(air_loop)
        end
      end
      ...
      def air_loop_typing(air_loop)
        @haystack_json.each do |entity|
          if entity[:id] == create_ref(air_loop.handle)
            infer_heating_type(entity)
            infer_cooling_type(entity)
          end
        end
      end
      ...
      def infer_heating_type(entity_to_type)
        @haystack_json.each do |entity|
          if (entity[:equipRef] == entity_to_type[:id]) && Set[:equip, :coil, :heating].subset?(entity.keys.to_set)
            if entity.key?(:dx)
      ...
      def infer_cooling_type(entity_to_type)
        @haystack_json.each do |entity|
          if (entity[:equipRef] == entity_to_type[:id]) && Set[:equip, :coil, :cooling].subset?(entity.keys.to_set)
            if entity.key?(:dx)
      ...
      def tag_fans(air_loop, sc)
        if sc.to_AirLoopHVACOutdoorAirSystem.is_initialized
          ...
          if supply_fan.is_initialized
            create_supply_fan(supply_fan, air_loop)
          end
      ...
      def tag_heating_and_cooling_components(air_loop, sc)
        if sc.to_AirLoopHVACUnitaryHeatPumpAirToAir.is_initialized
          ...
          if heating_coil.initialized
            create_dx_heating_coil(heating_coil, air_loop)
          end
          if cooling_coil.initialized
            create_dx_cooling_coil(cooling_coil, air_loop)
          end
        end
      end
